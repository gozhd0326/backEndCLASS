...
public class AuthController {
    ...
    public ResponseEntity<BaseResponse<TokenDto>> authorize(@RequestBody @Valid LoginDto loginDto) { //userName과 password를 받아옴(loginDto) 그렇게 로그인을 하고 나면 Token을 줌
        ...
        SecurityContextHolder.getContext().setAuthentication(authentication); // 현재 누가 어떠한 권한으로 사용하는지 확인하기 위해 
        //  SecurityContextHolder는 싱글턴(처음 메모리 생성 위치에서 변하지 않음)임! 현재 authentication(유저네임과 권한이 들어있음)을 저장 
        // 저장소 위치가 하나여서 매번 새롭게 갱신됨.(토큰 A롤 보내다가 토큰 B로 보낼경우 새롭게 갱신됨)
        // 도메인별로 관리함.(쓰레드(프로세스덩어리)를 만들어서 다 따로 관리함)
