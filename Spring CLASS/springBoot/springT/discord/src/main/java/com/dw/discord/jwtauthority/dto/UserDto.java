...
public class UserDto {
	...
	@JsonProperty(access = JsonProperty.Access.WRITE_ONLY) // dto 형태에서 읽을 수 없다. //USERDTO에서 JSOM형태로 응답을 했되 password를 넣지 않음. 
	@NotNull
	@NotBlank
	@Pattern(regexp = "^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@#$%^&*!])[A-Za-z\\d@#$%^&*!]{8,20}$",
			message = "영문 숫자 특수문자를 포함한 8~20자리로 입력해주세요") // 받을 때의 규칙
