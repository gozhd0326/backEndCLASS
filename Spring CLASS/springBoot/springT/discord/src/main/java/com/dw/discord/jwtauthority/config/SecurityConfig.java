...
public class SecurityConfig {
    ...
    public PasswordEncoder passwordEncoder() { // PasswordEncoder는 라이브러리가 사용하는 암호화 방식을 알려주는 것임. // Provider는 password를 암호화 해서 비교해야 하기 때문에 암호화 하는 방식을 알려줌
        return new BCryptPasswordEncoder(); // 라이브러리에 BCrypt를 사용할거라고 알려줌 그렇기 때문에 retrun값만 지정 (빅크립트)
    }


    @Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception { 
    	// 코드를 계속 이어서 사슬구조처럼 만들어놓음.(함수형 프로그램의 방식) //환경 설정을 하기 위해선 반드시 SecurityFilterChain을 만들어야 됨(필터체인)
        http //객체 
...
//                        .requestMatchers("/api/board").permitAll()
                ...
                .addFilterBefore( // 토큰을 까서 보면 안에 있는 어떤 내용을 어떻게 처리할지에 대한 내용
                        new JwtFilter(tokenProvider), // 토큰을 처리할 수 있는 방법에대한 커스터 마이징 // 필터빈을 상속 받도록 하기 위해 반드시 지정
